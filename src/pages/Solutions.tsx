...
const PAIN_POINTS = [
  {
    ...
    before: {
      ...
      bullets: ['5-step human approval chain', 'No automated rollback trigger', 'Blind to cross-service impact'],
    },
    after: {
      ...
      bullets: ['Canary → progressive → full in one command', 'Auto-rollback when p99 exceeds threshold', 'Full deploy lineage for every release'],
    },
  ...
  },
  {
    ...
    before: {
      ...
      bullets: ['Rollback = manual re-deploy', 'No automated health gate', 'On-call alert, not proactive detection'],
    },
    after: {
      label: 'With Titan Rollout',
      ...
      bullets: ['One-click or API-triggered rollback', 'Pre-warmed previous release always available', 'Automatic trigger from Datadog / Prometheus alert'],
    },
    link: '/products/titan-rollout',
    linkLabel: 'Explore Titan Rollout →',
  },
  {
    ...
    before: {
      ...
      bullets: ['Manual failover runbooks', 'No global health visibility', 'RTO measured in hours, not seconds'],
    },
    after: {
      ...
      bullets: ['Sub-second automated failover', 'Declarative cross-region policy (HCL / YAML)', 'RTO < 30 s for >99.9% of incidents'],
    },
  ...
  },
  {
    ...
    before: {
      ...
      bullets: ['Risk assessed after the fact', 'No dependency map at deploy time', 'SLO breaches, not warnings, are the signal'],
    },
    after: {
      ...
      bullets: ['Risk score on every PR, not just post-deploy', 'Blast-radius map across all dependencies', 'SLO guardrails block high-risk deploys automatically'],
    },
...
const PERSONAS = [
  {
    ...
    description: 'Stop being the deployment bottleneck. DeployTitan gives platform teams a single control plane to define deployment policies, enforce SLO guardrails, and let product teams self-serve — safely.',
    wins: [
  ...
  },
  {
    ...
    description: 'SREs use DeployTitan to turn manual escalation chains into automated responses. Define your error budget — let Titan Shield and Titan Rollout protect it.',
    wins: [
  ...
  },
  {
    ...
    description: 'DevOps teams spend too much time stitching together CI/CD scripts, rollback runbooks, and alert routing. DeployTitan replaces that scaffolding with a single declarative deploy layer.',
    wins: [
  ...
  return (
    <div className="py-16 border-b border-line last:border-b-0 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16" data-reveal>
      ...
      <div className="sharp-card border border-primary/20 p-6 bg-primary/[0.03]">
        ...
        <a href={pair.link} className="text-sm font-medium text-primary hover:text-primary-light transition-colors">
          {pair.linkLabel}
        </a>
      </div>
    </div>
...
export default function Solutions() {
  useDocumentMeta(
    'Solutions — DeployTitan',
    'See how DeployTitan solves slow releases, missing rollbacks, multi-cloud complexity, and blind deploy risk — for Platform Engineering, SRE, and DevOps teams.'
  )
  useScrollReveal()

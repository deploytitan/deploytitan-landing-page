    ...
    <>
      ...
      <section className="blueprint-grid pt-28 pb-20 border-b border-line">
        <div className="max-w-4xl mx-auto px-6" data-reveal>
          <p className="text-xs font-mono tracking-widest uppercase text-primary mb-4">Titan Sentinel</p>
          <h1 className="text-4xl lg:text-5xl font-semibold text-surface leading-tight mb-6">
            Know your risk<br className="hidden md:block" /> before you ship.
          </h1>
          <p className="text-lg text-muted leading-relaxed max-w-2xl mb-8">
            Titan Sentinel scores every release against your live dependency graph, calculates blast radius, and enforces SLO guardrails — before traffic ever shifts.
          </p>
      ...
      <section className="py-24 border-t border-line">
        <div className="max-w-6xl mx-auto px-6">
          <div className="mb-12" data-reveal>
            ...
            <h2 className="text-2xl lg:text-3xl font-semibold text-surface mb-3">Risk scoring in your CI pipeline in 5 minutes.</h2>
            <p className="text-muted max-w-xl">Add one GitHub Actions step. Get risk scores, blast-radius maps, and SLO impact reports on every PR — automatically.</p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8" data-reveal>
            <div className="flex flex-col gap-6">
              <div>
                <p className="text-xs font-mono text-muted uppercase tracking-wider mb-3">Install CLI</p>
                <InstallTabs />
              </div>
              <div>
                <p className="text-xs font-mono text-muted uppercase tracking-wider mb-3">CLI risk check</p>
                <CodeBlock code={CLI_CODE} lang="bash" filename="terminal" />
              </div>
            </div>
            <div>
              <p className="text-xs font-mono text-muted uppercase tracking-wider mb-3">GitHub Actions integration</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {CROSS_LINKS.map((l) => (
              <a
                key={l.href}
                href={l.href}
                className="sharp-card border border-line p-5 flex flex-col gap-1.5 hover:border-primary/30 hover:bg-white/[0.02] transition-colors"
                data-reveal
              >

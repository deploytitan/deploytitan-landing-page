...
const CLOUDS = [
  { name: 'AWS', color: 'text-[#FF9900]', bg: 'bg-[#FF9900]/10', adapters: ['ALB', 'Lambda Edge', 'ECS'] },
  { name: 'GCP', color: 'text-[#4285F4]', bg: 'bg-[#4285F4]/10', adapters: ['Cloud Run', 'Cloud Load Balancing', 'GKE Ingress'] },
  { name: 'Azure', color: 'text-[#0078D4]', bg: 'bg-[#0078D4]/10', adapters: ['Azure Front Door', 'AKS Ingress', 'Application Gateway'] },
  { name: 'Edge', color: 'text-orange-400', bg: 'bg-orange-400/10', adapters: ['Cloudflare', 'Fastly', 'CloudFront'] },
]
...
const SCENARIOS = [
  ...
  },
  {
    ...
    after: 'Run DR drill mode in Shield — simulate failover without real traffic impact. Document results automatically.',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
...
export default function SolutionMultiCloudResilience() {
  useDocumentMeta(
    'Multi-Cloud Resilience — DeployTitan',
    'Automated cross-cloud failover with sub-30s RTO. Titan Shield watches every region and shifts traffic before your on-call wakes up.'
  )
  useScrollReveal()
  ...
  return (
    <>
      ...
      <section className="blueprint-grid pt-28 pb-20 border-b border-line">
        <div className="max-w-4xl mx-auto px-6" data-reveal>
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs text-primary font-mono">Multi-Cloud Resilience</span>
          </div>
          ...
          <h1 className="text-4xl lg:text-5xl font-semibold text-ink leading-tight mb-5">
            Failover in seconds.<br className="hidden md:block" /> Not in war-room hours.
          </h1>
          <p className="text-lg text-ink-secondary leading-relaxed max-w-2xl mb-8">
            AWS us-east-1 goes down and your on-call is still waking up. Titan Shield has already shifted traffic. Declarative cross-cloud routing, fully automated failover, and a DR drill mode that actually prepares you for the real thing.
          </p>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <Link to="/products/titan-shield" className="text-sm font-medium text-primary hover:text-primary-dark transition-colors">
      ...
      <section className="py-20 border-b border-line">
        <div className="max-w-6xl mx-auto px-6">
          <div className="mb-12" data-reveal>
            <p className="text-xs font-mono tracking-widest uppercase text-primary mb-3">Coverage</p>
            <h2 className="text-2xl lg:text-3xl font-semibold text-ink mb-2">One control plane. Every cloud.</h2>
            <p className="text-ink-secondary text-sm max-w-lg">The Titan Shield controller runs in your infrastructure and speaks native L7 protocols — no proxy layer, no vendor lock-in.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4" data-reveal>
            {CLOUDS.map((cloud) => (
              <div key={cloud.name} className="sharp-card border border-line p-6 hover:border-primary/20 transition-all">
                <div className={`inline-flex items-center gap-2 px-2 py-1 rounded-sm mb-4 ${cloud.bg}`}>
      ...
      <section className="py-24 border-b border-line">
        <div className="max-w-6xl mx-auto px-6">
          <div className="mb-12" data-reveal>
            <p className="text-xs font-mono tracking-widest uppercase text-primary mb-3">Scenarios</p>
            <h2 className="text-2xl lg:text-3xl font-semibold text-ink">Every failure mode, handled automatically.</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {SCENARIOS.map((s, i) => (
              <div
                key={s.title}
                className="sharp-card border border-line p-7 hover:border-primary/20 transition-all"
                data-reveal
                data-reveal-delay={String(i)}
              >
                <div className="flex items-center gap-3 text-primary mb-4">{s.icon}<h3 className="font-semibold text-ink text-sm">{s.title}</h3></div>
      ...
      <section className="py-20 border-b border-line bg-surface-alt/30">
        <div className="max-w-3xl mx-auto px-6">
          <div className="mb-8" data-reveal>
            <p className="text-xs font-mono tracking-widest uppercase text-primary mb-3">Policy as code</p>
            <h2 className="text-2xl font-semibold text-ink mb-2">Define failover policy once. Never touch it again.</h2>
          </div>
          <div className="sharp-card border border-line overflow-hidden" data-reveal>
            ...
            <div className="p-5 font-mono text-sm leading-relaxed text-ink-secondary">
              <p><span className="text-primary">failover_policy</span> <span className="text-ink">"production"</span> {'{'}</p>
              <p className="ml-4 text-ink-secondary">primary_region  = <span className="text-signal-success">"aws/us-east-1"</span></p>
              <p className="ml-4 text-ink-secondary">failover_regions = [<span className="text-signal-success">"gcp/us-central1"</span>, <span className="text-signal-success">"aws/eu-west-1"</span>]</p>
              <p className="ml-4 mt-2 text-ink-secondary">trigger {'{'}</p>
              <p className="ml-8 text-ink-secondary">error_rate_threshold = <span className="text-primary">0.05</span></p>
              <p className="ml-8 text-ink-secondary">p99_latency_ms       = <span className="text-primary">500</span></p>
              <p className="ml-8 text-ink-secondary">consecutive_failures = <span className="text-primary">3</span></p>
              <p className="ml-4">{'}'}</p>
              <p className="ml-4 mt-2 text-ink-secondary">rto_target_seconds = <span className="text-primary">30</span></p>
              <p>{'}'}</p>
            </div>
          </div>
        </div>

...
export default function ForDevOps() {
  useDocumentMeta(
    'DeployTitan for DevOps & Release Engineering',
    'Ship Friday. Sleep Saturday. One CLI command replaces 200 lines of pipeline YAML — with automatic rollback built in.'
  )
  useScrollReveal()
    ...
    <>
      <section className="blueprint-grid pt-28 pb-20 border-b border-line">
        <div className="max-w-4xl mx-auto px-6" data-reveal>
          <p className="text-xs font-mono tracking-widest uppercase text-primary mb-4">For DevOps & Release Engineering</p>
          <h1 className="text-4xl lg:text-5xl font-semibold text-ink leading-tight mb-5">
            Ship Friday.<br className="hidden md:block" /> Sleep Saturday.
          </h1>
          <p className="text-lg text-ink-secondary leading-relaxed max-w-2xl mb-8">
            One `dt deploy` command replaces 200 lines of pipeline YAML, a rollback runbook, and a "please don't deploy on Friday" Slack rule. Automatic rollback, canary stepping, and cross-cloud support — wired together.
          </p>
          <div className="flex flex-wrap items-center gap-4">
            ...
            <Link to="/solutions/progressive-delivery" className="text-sm font-medium text-primary hover:text-primary-dark transition-colors">
      ...
      <section className="py-24 border-b border-line">
        <div className="max-w-5xl mx-auto px-6">
          <div className="mb-12" data-reveal>
            ...
            <h2 className="text-2xl font-semibold text-ink">Less pipeline YAML. More confidence.</h2>
          </div>
          <div className="flex flex-col gap-6">
            {FEATURES.map((f, i) => (
              <div
                key={f.title}
                className="grid grid-cols-1 lg:grid-cols-2 gap-6 sharp-card border border-line p-7 hover:border-primary/20 transition-all"
                data-reveal
              ...
              >
                ...
                <div className="font-mono text-xs bg-surface-alt border border-line p-4 flex items-center" style={{ borderRadius: '2px' }}>
      ...
      <section className="py-20 border-b border-line bg-surface-alt/30">
        <div className="max-w-5xl mx-auto px-6" data-reveal>
          <p className="text-xs font-mono tracking-widest uppercase text-primary mb-4">CI/CD integrations</p>
          <h2 className="text-2xl font-semibold text-ink mb-5">Works with your existing pipeline.</h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {['GitHub Actions', 'GitLab CI', 'CircleCI', 'Jenkins', 'Buildkite', 'ArgoCD', 'Terraform', 'Pulumi'].map((ci) => (
              <div key={ci} className="sharp-card border border-line p-3 text-center text-xs font-medium text-ink-secondary hover:border-primary/20 hover:text-ink transition-all">
                {ci}
              </div>
            ))}
          </div>

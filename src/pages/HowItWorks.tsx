...
const SECURITY_PROPS = [
  { label: 'Your data plane', body: 'The controller runs in your VPC or cluster. Traffic never touches our infrastructure.' },
  { label: '<3ms overhead', body: 'The controller adds less than 3ms to routing decisions. Pure passthrough for routing logic.' },
  { label: '~70MB distroless', body: 'The controller container is distroless and statically linked. Minimal attack surface.' },
  { label: 'Declarative config', body: 'No imperative scripts. Every change is a policy diff, code-reviewed, and audited.' },
]

export default function HowItWorks() {
  useDocumentMeta(
    'How It Works — DeployTitan',
    'Your data plane, our brain. The DeployTitan controller runs in your infrastructure and translates deployment intent into native L7 config.'
  )
  useScrollReveal()
    ...
    <>
      ...
      <section className="blueprint-grid pt-28 pb-20 border-b border-line">
        <div className="max-w-4xl mx-auto px-6 text-center" data-reveal>
          <p className="text-xs font-mono tracking-widest uppercase text-primary mb-4">Architecture</p>
          <h1 className="text-4xl lg:text-5xl font-semibold text-ink leading-tight mb-5">
            Your data plane.<br className="hidden md:block" /> Our brain.
          </h1>
          <p className="text-lg text-ink-secondary leading-relaxed max-w-2xl mx-auto mb-8">
            DeployTitan doesn't sit in your traffic path. A lightweight controller runs inside your infrastructure and translates declarative deployment policy into native L7 config — for every platform you already use.
          </p>
          <div className="flex flex-wrap items-center justify-center gap-4">
            ...
            <Link to="/docs" className="text-sm font-medium text-primary hover:text-primary-dark transition-colors">
              Read the docs →
            </Link>
          </div>
        </div>
      ...
      <section className="py-24 border-b border-line">
        <div className="max-w-5xl mx-auto px-6">
          <div className="mb-14" data-reveal>
            ...
            <h2 className="text-2xl lg:text-3xl font-semibold text-ink mb-2">Three layers. Zero lock-in.</h2>
          </div>
          ...
          <div className="space-y-3" data-reveal>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <span className="font-mono text-[9px] text-ink-quaternary border border-line px-1.5 py-0.5" style={{ borderRadius: '2px' }}>DeployTitan SaaS</span>
                  </div>
                  <p className="text-xs text-ink-secondary">Policy storage, risk analysis, deploy orchestration, audit log, dashboard. Hosted by us — your traffic never passes through.</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-center">
              <div className="flex flex-col items-center gap-1">
                ...
                <svg width="10" height="8" viewBox="0 0 10 8" fill="none"><path d="M5 8L0 0h10L5 8z" fill="rgba(201,168,76,0.3)"/></svg>
                <p className="text-[9px] font-mono text-ink-quaternary">Polls for deploy intent (HTTPS)</p>
              </div>
            </div>
            ...
            <div className="sharp-card border border-line bg-surface-alt/30 p-6">
              <div className="flex items-start gap-4">
                <div className="w-10 h-10 border border-line flex items-center justify-center shrink-0">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" className="text-ink-secondary">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <span className="font-mono text-[9px] text-signal-success/80 border border-signal-success/30 bg-signal-success/5 px-1.5 py-0.5" style={{ borderRadius: '2px' }}>Runs in your infra</span>
                  </div>
                  <p className="text-xs text-ink-secondary mb-3">Distroless ~70MB container. Polls the API, executes routing changes natively. Adds &lt;3ms overhead. Runs in your VPC — traffic never leaves.</p>
                  <div className="flex flex-wrap gap-2">
                    {['docker run deploytitan/controller', 'helm install titan-controller deploytitan/controller', 'Cloud Run service'].map(c => (
                      <span key={c} className="font-mono text-[10px] text-ink-tertiary bg-surface-alt border border-line px-2 py-0.5" style={{ borderRadius: '2px' }}>{c}</span>
                    ))}
                  </div>
                </div>
              </div>
            ...
            <div className="flex justify-center">
              <div className="flex flex-col items-center gap-1">
                <div className="w-px h-4 bg-primary/20" />
                <svg width="10" height="8" viewBox="0 0 10 8" fill="none"><path d="M5 8L0 0h10L5 8z" fill="rgba(201,168,76,0.3)"/></svg>
                <p className="text-[9px] font-mono text-ink-quaternary">Native L7 config (no proxy)</p>
              </div>
            </div>

            {/* Layer 3: Platforms */}
            <div className="sharp-card border border-line p-6">
              <p className="text-[11px] font-mono tracking-widest uppercase text-ink-quaternary mb-4">Your existing platforms</p>
      ...
      <section className="py-24 border-b border-line">
        <div className="max-w-5xl mx-auto px-6">
          <div className="mb-14" data-reveal>
            <p className="text-xs font-mono tracking-widest uppercase text-primary mb-3">Walkthrough</p>
            <h2 className="text-2xl lg:text-3xl font-semibold text-ink">From `dt deploy` to production in five steps.</h2>
          </div>
      ...
      <section className="py-20 border-b border-line bg-surface-alt/30">
        <div className="max-w-5xl mx-auto px-6">
          <div className="mb-10" data-reveal>
            <p className="text-xs font-mono tracking-widest uppercase text-primary mb-3">Security model</p>
            <h2 className="text-2xl font-semibold text-ink mb-2">Designed to satisfy your security team.</h2>
            <p className="text-ink-secondary text-sm max-w-lg">Every architectural decision was made with enterprise security review in mind.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4" data-reveal>
            {SECURITY_PROPS.map((p) => (
              <div key={p.label} className="sharp-card border border-line p-5 flex items-start gap-4">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-signal-success shrink-0 mt-0.5">
          ...
          <div className="mt-6" data-reveal>
            <Link to="/security" className="text-sm text-primary hover:text-primary-dark transition-colors font-medium">

...
const pressItems = [
  ...
  },
  {
    ...
    headline: 'How DeployTitan is bringing progressive delivery to teams without dedicated platform engineering',
    href: '#',
  },
  {
    date: 'February 2026',
...
export default function Press() {
  useDocumentMeta('Press', 'Press releases, media coverage, and press kit resources for DeployTitan.')
  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <section className="border-b border-line blueprint-grid">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-16 lg:py-20">
          <div className="max-w-2xl">
            <span className="font-mono text-[11px] text-ink-quaternary uppercase tracking-widest">Press</span>
            ...
            <p className="mt-5 text-lg text-ink-secondary leading-relaxed max-w-lg">
              Press releases, coverage, and resources for journalists and analysts covering DeployTitan.
            </p>
            <div className="mt-8">
              <a
      ...
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 py-14">
        <span className="font-mono text-[10px] text-ink-quaternary uppercase tracking-widest">Coverage</span>
          ...
          {pressItems.map((item) => (
            ...
            >
              ...
              <div className="flex flex-col gap-1">
                <span className="font-mono text-[10px] text-primary uppercase tracking-widest">{item.publication}</span>
                <h3 className="text-base text-ink group-hover:text-primary-dark transition-colors leading-snug">{item.headline}</h3>
              </div>
              <svg className="ml-auto shrink-0 opacity-30 group-hover:opacity-70 transition-opacity mt-1" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
      ...
      <section className="border-t border-line bg-surface-alt">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-14">
          <span className="font-mono text-[10px] text-ink-quaternary uppercase tracking-widest">Press kit</span>
          <h2 className="mt-3 text-2xl font-display font-medium text-ink tracking-tight">Assets & boilerplate</h2>
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { label: 'Logo pack (SVG, PNG)', desc: 'Dark, light, and monochrome variants' },
              { label: 'Company boilerplate', desc: 'One-paragraph and two-paragraph descriptions' },
              { label: 'Founder headshots', desc: 'High-resolution press photos' },
            ].map((asset) => (
              <div key={asset.label} className="sharp-card bg-surface p-6">
                <h3 className="text-sm font-medium text-ink">{asset.label}</h3>
                <p className="mt-1 text-xs text-ink-secondary">{asset.desc}</p>
                <a href="mailto:press@deploytitan.com" className="mt-4 inline-flex items-center text-xs text-primary font-medium hover:text-primary-dark transition-colors">
                  Request via email →
                </a>
              </div>
            ))}

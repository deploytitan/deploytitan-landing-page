...
const partners = [
  {
    category: 'Cloud',
    items: [
      { name: 'Amazon Web Services', description: 'EKS, ECS, CodePipeline, and CloudFormation integrations.' },
      { name: 'Google Cloud', description: 'GKE, Cloud Run, and Cloud Deploy integrations.' },
      { name: 'Microsoft Azure', description: 'AKS, Azure Pipelines, and Azure DevOps integrations.' },
    ],
  },
  {
    category: 'CI/CD',
    items: [
      { name: 'GitHub', description: 'Native GitHub Actions integration and Deployments API support.' },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'PagerDuty', description: 'Incident correlation and deployment pause on active incidents.' },
    ],
  },
]

export default function Partners() {
  useDocumentMeta('Partners', 'DeployTitan technology partners — cloud providers, CI/CD platforms, and observability tools.')
  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <section className="border-b border-line blueprint-grid">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-16 lg:py-20">
          <div className="max-w-2xl">
            <span className="font-mono text-[11px] text-ink-quaternary uppercase tracking-widest">Partners</span>
            ...
            <p className="mt-5 text-lg text-ink-secondary leading-relaxed max-w-lg">
              DeployTitan integrates with the cloud providers, CI/CD platforms, and observability tools your team already relies on.
            </p>
      ...
      {/* Partner grid */}
      {partners.map((group) => (
        <section key={group.category} className="border-t border-line">
          <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-12">
            <span className="font-mono text-[10px] text-ink-quaternary uppercase tracking-widest">{group.category}</span>
            <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {group.items.map((p) => (
                <div key={p.name} className="sharp-card bg-surface-alt p-6">
                  <h3 className="text-sm font-medium text-ink">{p.name}</h3>
                  <p className="mt-1.5 text-xs text-ink-secondary leading-relaxed">{p.description}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="border-t border-line bg-surface-alt">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-14">
          <div className="max-w-xl">
            <span className="font-mono text-[10px] text-ink-quaternary uppercase tracking-widest">Partner program</span>
            <h2 className="mt-3 text-2xl font-display font-medium text-ink tracking-tight">Integrate with DeployTitan</h2>
            <p className="mt-3 text-sm text-ink-secondary leading-relaxed">
              We work closely with technology partners to build deep, well-tested integrations. If you're a cloud provider, observability vendor, or DevOps platform, we'd love to talk.
            </p>
            <a
              href="mailto:partners@deploytitan.com"

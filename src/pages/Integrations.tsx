...
export default function Integrations() {
  useDocumentMeta('Integrations', 'Connect DeployTitan to Kubernetes, GitHub Actions, Datadog, AWS, Slack, and 20+ more tools.')
      ...
      <section className="border-b border-line blueprint-grid">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-16 lg:py-20">
          <div className="max-w-2xl">
            <span className="font-mono text-[11px] text-ink-quaternary uppercase tracking-widest">Integrations</span>
            <h1 className="mt-3 text-4xl sm:text-5xl font-display font-medium tracking-tight text-ink leading-[1.1]">
              Plugs into your<br />entire stack
            </h1>
            <p className="mt-5 text-lg text-ink-secondary leading-relaxed max-w-lg">
              DeployTitan works with the tools you already use — from your CI pipeline and cloud provider
              to your observability stack and incident management platform.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 py-12">
        {visible.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-ink-secondary font-mono text-sm">No integrations match your filter.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {visible.map((integration) => (
              ...
              >
                ...
                <h3 className="text-sm font-medium text-ink group-hover:text-primary transition-colors">{integration.name}</h3>
                <p className="mt-1 text-xs text-ink-tertiary leading-relaxed line-clamp-2">{integration.description}</p>
                <div className="mt-3 flex flex-wrap gap-1">
                  {integration.products.map((p) => (
                    <span key={p} className="font-mono text-[9px] text-ink-quaternary border border-line px-1.5 py-0.5" style={{ borderRadius: '2px' }}>
                      {p}
                    </span>
                  ))}
                </div>
      ...
      <section className="border-t border-line bg-surface-alt">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-12 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            ...
            <p className="mt-1 text-sm text-ink-secondary">We ship new integrations every sprint. Request one or build your own with the REST API.</p>
          </div>

...
const items: RoadmapItem[] = [
  // Shipped
  {
    ...
    description: 'Progressive traffic shifting with SLO-gated auto-promotion at 10%, 25%, 50%, 100%.',
    status: 'shipped',
    quarter: 'Q1 2026',
  ...
  },
  {
    ...
    description: 'Deployment scope controls to limit blast radius across AWS, GCP, and Azure simultaneously.',
    status: 'shipped',
    quarter: 'Q1 2026',
  ...
  },
  {
    ...
    description: 'First-class action for triggering, monitoring, and rolling back deployments from your CI workflow.',
    status: 'shipped',
    quarter: 'Q4 2025',
    tags: ['Integrations'],
  },
  ...
  // In progress
  {
    ...
    description: 'Deep integration with ArgoCD — use DeployTitan rollout policies with GitOps workflows.',
    status: 'in-progress',
    quarter: 'Q2 2026',
  ...
  },
  {
    ...
    description: 'Role-based access control with full audit trail — who deployed what, when, and from where.',
    status: 'in-progress',
    quarter: 'Q2 2026',
    tags: ['Security'],
  },
  // Planned
  {
    ...
    description: 'Bring infrastructure changes under the same progressive delivery model as application deployments.',
    status: 'planned',
    quarter: 'Q3 2026',
    tags: ['IaC'],
  },
  {
    ...
    description: 'Automated weekly SLA reports — MTTR, MTBF, deployment frequency, and change failure rate.',
    status: 'planned',
    quarter: 'Q3 2026',
    tags: ['Observability'],
  },
  {
    ...
    description: 'Built-in feature flags tied to deployment steps — ship dark, then flip the switch.',
    status: 'planned',
    quarter: 'Q3 2026',
    tags: ['Core'],
  },
  // Considering
  {
    ...
    description: 'Use historical deployment signals to predict and pre-emptively roll back risky releases.',
    status: 'considering',
    tags: ['AI', 'Core'],
  },
  {
...
const statusConfig: Record<Status, { label: string; dot: string; badge: string }> = {
  shipped: {
    label: 'Shipped',
    dot: 'bg-emerald-500',
    badge: 'text-emerald-700 bg-emerald-50 border-emerald-200 dark:text-emerald-400 dark:bg-emerald-950/30 dark:border-emerald-800',
  },
  'in-progress': {
    label: 'In progress',
    dot: 'bg-primary',
...
export default function Roadmap() {
  useDocumentMeta('Roadmap', "What we're building at DeployTitan — shipped, in progress, planned, and on our radar.")
  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <section className="border-b border-line blueprint-grid">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-16 lg:py-20">
          <div className="max-w-2xl">
            <span className="font-mono text-[11px] text-ink-quaternary uppercase tracking-widest">Roadmap</span>
            ...
            <p className="mt-5 text-lg text-ink-secondary leading-relaxed max-w-lg">
              A live view of shipped features, what's in active development, and what's coming next. We ship fast — check the changelog for detailed release notes.
            </p>
      ...
      <section className="border-b border-line bg-surface-alt">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 py-4 flex flex-wrap items-center gap-6">
          {order.map((s) => {
      ...
      {/* Roadmap items */}
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 py-14 space-y-14">
          ...
          return (
            <div key={status}>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="font-display text-xl font-medium text-ink tracking-tight">{cfg.label}</h2>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {group.map((item) => (
                  <div key={item.title} className="sharp-card bg-surface-alt p-6 flex flex-col gap-3">
                    <div className="flex items-start justify-between gap-3">
                      <h3 className="text-sm font-medium text-ink leading-snug">{item.title}</h3>
                      {item.quarter && (
                        <span className="font-mono text-[10px] text-ink-quaternary shrink-0">{item.quarter}</span>
                      )}
                    </div>
                    <p className="text-xs text-ink-secondary leading-relaxed">{item.description}</p>
                    <div className="flex flex-wrap gap-1.5 mt-auto pt-1">
                      {item.tags.map((tag) => (
                        <span key={tag} className="font-mono text-[10px] px-1.5 py-0.5 border border-line text-ink-tertiary" style={{ borderRadius: '2px' }}>
                          {tag}
                        </span>
                      ))}
                    </div>

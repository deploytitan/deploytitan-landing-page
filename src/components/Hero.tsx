    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-line/50 bg-surface-alt/60 shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono text-ink-tertiary tracking-wider uppercase">DeployTitan — Traffic Control</span>
        </div>
        <div className="flex items-center gap-2">
          <div
            className="w-1.5 h-1.5 transition-all duration-500"
            style={{ backgroundColor: info.color, borderRadius: '1px', boxShadow: `0 0 6px ${info.color}50` }}
          />
          <span className="text-[9px] font-mono transition-colors duration-500" style={{ color: info.color }}>
            {info.text}
          </span>
        </div>
      </div>
      ...
      <div className="flex-1 p-5 flex flex-col gap-4">
        ...
        <div className="space-y-3">
          <div className="text-[9px] font-mono text-ink-quaternary uppercase tracking-[0.08em]">Active versions</div>
          {versions.map((v) => (
            <div key={v.id}>
        ...
        <div className="flex items-center justify-between">
          <div className="text-[9px] font-mono text-ink-quaternary uppercase tracking-[0.08em]">Policy</div>
          <div className="flex items-center gap-2">
        ...
        <div className="flex-1 flex flex-col justify-end">
          <div className="text-[9px] font-mono text-ink-quaternary uppercase tracking-[0.08em] mb-1.5">Event log</div>
          <div className="space-y-1">
            {logLines.map((line, i) => (
              <div key={i} className="log-entry flex items-center gap-2">
                <div className="w-1 h-1 shrink-0" style={{ backgroundColor: line.color, borderRadius: '0.5px' }} />
        ...
        <div className="max-w-xl">
          ...
          <div data-reveal className="mb-8">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 border border-line bg-surface" style={{ borderRadius: '2px' }}>
              ...
              <span className="text-xs text-ink-secondary font-mono">Live public demo available</span>
            </div>
          </div>
          ...
          <div data-reveal data-reveal-delay="4" className="mt-10 pt-10 border-t border-line">
            <p className="text-xs uppercase tracking-widest mb-4 font-mono" style={{ color: PRIMARY }}>Works with your existing stack</p>
            <div className="flex flex-wrap items-center gap-4 text-sm text-ink-secondary">
              {['Kubernetes', 'GCP Cloud Run', 'AWS Lambda', 'Azure AKS', 'Docker Swarm'].map((p) => (
                <div key={p} className="flex items-center gap-2 opacity-50 hover:opacity-100 transition-opacity">
                  <div className="w-1 h-1" style={{ backgroundColor: `${PRIMARY_RGBA},0.3)`, borderRadius: '0.5px' }} />

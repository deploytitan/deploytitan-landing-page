    ...
    <section className="py-16 lg:py-20 border-t border-line bg-surface-alt/40 relative" ref={ref}>
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12 relative">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h2 data-reveal data-reveal-delay="1" className="font-display font-medium text-4xl lg:text-5xl tracking-[-0.022em] leading-[1.1] mb-6">
            The problem isn't your team—<br />
            <span className="text-ink-secondary">it's your control.</span>
          </h2>

          <p data-reveal data-reveal-delay="2" className="text-lg text-ink-secondary leading-relaxed mb-10 max-w-2xl mx-auto">
            Most teams rely on deployments to make changes. That means every fix, rollback, or adjustment goes through the same heavy pipeline:
          </p>
        </div>
            ...
            {heavySteps.map((step, i) => (
              <div key={step.label} className="flex flex-col items-stretch">
                ...
                >
                  ...
                  <div className="text-[9px] font-mono text-ink-tertiary ml-auto leading-tight text-right">{step.desc}</div>
                </div>
                {i < heavySteps.length - 1 && (
                  <div className="flex items-center justify-center h-4 border-l border-r border-line bg-surface-alt/60 -my-px relative z-10">
                    <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                      <path d="M5 2v6M3 6l2 2 2-2" stroke="rgba(8,5,3,0.2)" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </div>
                )}
              </div>
            ...
            {heavySteps.map((step, i) => (
              <div key={step.label} className="flex items-center flex-1 min-w-0">
                ...
                >
                  ...
                  <div className="text-[11px] font-mono font-medium text-ink truncate">{step.label}</div>
                  <div className="text-[9px] font-mono text-ink-tertiary mt-1 leading-tight">{step.desc}</div>
                </div>
                {i < heavySteps.length - 1 && (
                  <div className="shrink-0 w-5 flex items-center justify-center bg-surface-alt border-t border-b border-line h-full -mx-px relative z-10">
                    <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                      <path d="M2 5h6M6 3l2 2-2 2" stroke="rgba(8,5,3,0.2)" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </div>
                )}
              </div>
          ...
          <div className="border border-line bg-white p-5 sm:p-8 lg:p-10" style={{ borderRadius: '2px' }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
              {burdens.map((b, i) => (
                ...
                >
                  <div className="text-xs font-mono font-medium text-signal-danger mb-1">{b.label}</div>
            ...
            <div className="pt-6 border-t border-line text-center">
              ...
              <p className="text-sm text-ink-secondary mt-1">
                ...
                <span className="font-medium" style={{ color: PRIMARY }}>control after the fact.</span>
              </p>
            </div>
          </div>
        </div>

'use client'

import Link from 'next/link'
import { Section } from './shared/Section'
import { Container } from './shared/Container'
import { BrandLogo } from './shared/BrandLogo'

export function Footer() {
  return (
    <Section as="footer" border="top" padding="none" className="relative">
      {/* Gold accent line */}
      <div className="via-primary/20 absolute top-0 left-1/2 h-px w-48 -translate-x-1/2 bg-gradient-to-r from-transparent to-transparent" />

      {/* Main grid */}
      <Container className="grid grid-cols-2 gap-10 py-16 md:grid-cols-4">
        {/* Brand */}
        <div className="col-span-2 flex flex-col gap-4 md:col-span-1">
          <Link href="/" className="flex min-h-11 items-center">
            <BrandLogo className="h-5 w-auto" />
          </Link>
          <p className="text-ink-secondary max-w-62.5 text-xs leading-relaxed">
            Release coordination and deployment safety for distributed engineering teams.
          </p>
          {/* Social icons */}
          <div className="mt-1 flex items-center gap-3">
            <a
              href="https://github.com/deploytitan"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="GitHub"
              className="text-ink-secondary hover:text-ink inline-flex min-h-11 min-w-11 items-center justify-center transition-colors"
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z" />
              </svg>
            </a>
            <a
              href="https://twitter.com/deploytitan"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="X / Twitter"
              className="text-ink-secondary hover:text-ink inline-flex min-h-11 min-w-11 items-center justify-center transition-colors"
            >
              <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
              </svg>
            </a>
            <a
              href="https://linkedin.com/company/deploytitan"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="LinkedIn"
              className="text-ink-secondary hover:text-ink inline-flex min-h-11 min-w-11 items-center justify-center transition-colors"
            >
              <svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor">
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
              </svg>
            </a>
          </div>
        </div>

        {/* Product column */}
        <div className="flex flex-col gap-4">
          <p className="mt-3 text-ink-secondary font-mono text-[10px] tracking-widest uppercase">
            Product
          </p>
          <ul className="flex flex-col gap-3">
            {(
              [
                { label: 'Titan Rollouts', to: '/products/titan-rollout' },
                { label: 'Join waitlist', to: '/waitlist' },
              ] as { label: string; to: string; status?: 'roadmap' }[]
            ).map((l) => (
              <li key={l.label}>
                <Link
                  href={l.to}
                  className="text-ink-secondary hover:text-ink inline-flex items-center gap-1.5 text-sm transition-colors"
                >
                  {l.label}
                  {l.status === 'roadmap' && (
                    <span className="text-ink-secondary font-mono text-xs">· Soon</span>
                  )}
                </Link>
              </li>
            ))}
          </ul>
        </div>

        {/* Support column */}
        <div className="flex flex-col gap-4">
          <p className="mt-3 text-ink-secondary font-mono text-[10px] tracking-widest uppercase">
            Resources
          </p>
          <ul className="flex flex-col gap-3">
            {[
              { label: 'Blog', to: '/blog' },
              { label: 'My journey', to: '/journey' },
            ].map((l) => (
              <li key={l.label}>
                <Link
                  href={l.to}
                  className="text-ink-secondary hover:text-ink inline-flex items-center text-sm transition-colors"
                >
                  {l.label}
                </Link>
              </li>
            ))}
          </ul>
        </div>

        {/* Company column */}
        <div className="flex flex-col gap-4">
          <p className="mt-3 text-ink-secondary font-mono text-[10px] tracking-widest uppercase">
            Company
          </p>
          <ul className="flex flex-col gap-3">
            {[
              { label: 'About', to: '/about' },
              { label: 'Contact', to: '/contact' },
            ].map((l) => (
              <li key={l.label}>
                <Link
                  href={l.to}
                  className="text-ink-secondary hover:text-ink inline-flex items-center text-sm transition-colors"
                >
                  {l.label}
                </Link>
              </li>
            ))}
          </ul>
        </div>
      </Container>

      {/* Bottom bar */}
      <div className="border-line border-t">
        <Container className="flex flex-col items-center justify-between gap-3 py-5 sm:flex-row">
          <span className="text-ink-secondary font-mono text-xs">
            &copy; 2026{" "}
            <Link
              href="https://astralforgelabs.com"
              target="_blank"
              rel="noreferrer"
              className="hover:text-ink inline-flex min-h-11 items-center transition-colors"
            >
              ASTRALFORGE LABS LLP
            </Link>
            . All rights reserved.
          </span>
          <div className="flex items-center gap-6">
            <Link
              href="/terms"
              className="text-ink-secondary hover:text-ink inline-flex min-h-11 min-w-11 items-center justify-center text-xs transition-colors"
            >
              Terms
            </Link>
            <Link
              href="/privacy"
              className="text-ink-secondary hover:text-ink inline-flex min-h-11 min-w-11 items-center justify-center text-xs transition-colors"
            >
              Privacy
            </Link>
            <Link
              href="/sitemap"
              className="text-ink-secondary hover:text-ink inline-flex min-h-11 min-w-11 items-center justify-center text-xs transition-colors"
            >
              Site Map
            </Link>
          </div>
        </Container>
      </div>
    </Section>
  )
}
